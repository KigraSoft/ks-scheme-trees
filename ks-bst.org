...
#+begin_src scheme
  ...
  (define-library (ks-bst)
    ...
    (import (scheme cxr))
    (import (srfi srfi-9))
    (export make insert print get insert-or-replace)
    (begin
...
**** FUNC: make - BST Initialization - v1
...
**** FUNC: make - BST Initialization - v2

#+begin_src scheme

  (define (make . key-type-or-compare-function)
    (display key-type-or-compare-function)
    (newline)
    (cond
     ((or (null? key-type-or-compare-function)
          (eqv? 'int (car key-type-or-compare-function)))
      (cons `('int ,bst-int-compare) '()))
     ((eqv? 'str (car key-type-or-compare-function))
      (cons `('str ,bst-str-compare) '()))
     (else
      (cons `('fun ,key-type-or-compare-function) '()))))
...
*** BST Compare Functions
**** FUNC: bst-int-compare

#+begin_src scheme

  (define (bst-int-compare opr val1 val2)
    (cond ((eq? opr #\<)
           (< val1 val2))
          ((eq? opr #\>)
           (> val1 val2))
...
**** FUNC: bst-str-compare

#+begin_src scheme

  (define (bst-str-compare opr val1 val2)
    (cond ((eq? opr #\<)
           (string<? val1 val2))
          ((eq? opr #\>)
           (string>? val1 val2))
...
! Need to update function below to use stored compare function
...
#+begin_src scheme
  ...
  (define (insert bst key datum)
    (if (null? (cdr bst))
        (set-cdr! bst (new-node key (list datum) '()))
        (let ((compare (cadar bst)))
          (let tree-insert ((tree-root (cdr bst)))
            (cond
             ((compare #\= (get-node-key tree-root) key)
              (set-node-datum! tree-root (append
                                          (list datum)
                                          (node-datum tree-root))))
             ((compare #\> (get-node-key tree-root) key)
              (if (null? (node-left tree-root))
                  (set-node-left! tree-root
                                  (new-node key (list datum) tree-root))
                  (tree-insert (node-left tree-root))))
             ((compare #\< (get-node-key tree-root) key)
              (if (null? (node-right tree-root))
                  (set-node-right! tree-root
                                   (new-node key (list datum) tree-root))
                  (tree-insert (node-right tree-root)))))))))

#+end_src

*** BST Insert Or Replace

**** FUNC: insert-or-replace

#+begin_src scheme

  (define (insert-or-replace bst key datum)
    (if (null? (cdr bst))
        (set-cdr! bst (new-node key (list datum) '()))
        (let ((compare (cadar bst)))
          (let tree-insert ((tree-root (cdr bst)))
            (cond
             ((compare #\= (get-node-key tree-root) key)
              (set-node-datum! tree-root (list datum)))
             ((compare #\> (get-node-key tree-root) key)
              (if (null? (node-left tree-root))
                  (set-node-left! tree-root
                                  (new-node key (list datum) tree-root))
...
** ks-bst-test-05.scm
:PROPERTIES:
:header-args:scheme: :tangle ./bin/ks-bst-test-05.scm :tangle-mode o755
:END:
...
#+begin_src scheme
  ...
  (define (main args)
    ...
    (display (get tst 18))
    (newline)
    (display "Get datum for 7:  ")
    (display (get tst 7))
    (newline)
    (insert-or-replace tst 7 "test7--test7")
    (display "Get datum for 7:  ")
    (display (get tst 7))
    (newline)
    (insert-or-replace tst 18 "test18")
    (display "Get datum for 18: ")
    (display (get tst 18))
    (newline))
...
** ks-bst-test-06.scm
:PROPERTIES:
:header-args:scheme: :tangle ./bin/ks-bst-test-06.scm :tangle-mode o755
:END:

First test for using strings as the key value.  Initial run is a failure do to "<" not being able to compare strings.
...
#+begin_src scheme
  ...
  (define (main args)
    ...
    (insert tst "E" "testE")
    (print tst)
    (display "Get datum for D:  ")
    (display (get tst "D"))
    (newline)
    (display "Get datum for A:  ")
    (display (get tst "A"))
    (newline)
    (display "Get datum for H:  ")
    (display (get tst "H"))
    (newline)
    (display "Get datum for G:  ")
    (display (get tst "G"))
    (newline)
    (display "Get datum for B:  ")
    (display (get tst "B"))
    (newline)
    (display "Get datum for C:  ")
    (display (get tst "C"))
    (newline)
    (display "Get datum for Z:  ")
    (display (get tst "Z"))
    (newline)
    (display "Get datum for E:  ")
    (display (get tst "E"))
    (newline)
    (insert-or-replace tst "E" "testE--testE")
    (display "Get datum for E:  ")
    (display (get tst "E"))
    (newline)
    (insert-or-replace tst "Z" "testZ")
    (display "Get datum for Z:  ")
    (display (get tst "Z"))
    (newline))
